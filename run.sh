...
# deduplicate queries
# this goes from ~60 million queries to ~8 million
...
# not actually using this for anything, just to get a feel of the scope of the most deuplicated queries
zcat data/2017-06-12_2017-07-09_all_uq.tsv.gz | sort -S 1000000 | uniq -cd | sort -nr | head -n100 > 2017-06-12_2017-07-09_top.tsv
# one query was performed 3.2 million times (identical, not counting anonymized strings) some sort of musicbrainz query


# get counts of the most common properties
zcat data/2017-06-12_2017-07-09_uniq.tsv.gz | grep -Po "<http://www.wikidata.org/[^>]*?>" | grep -Po "P[^>]*" |
sort | uniq -c | sort -nr > prop_count.txt

# get counts of the most common items
zcat data/2017-06-12_2017-07-09_uniq_uq.tsv.gz | grep -Po "<http://www.wikidata.org/[^>]*?>" | grep -Po "Q[^>]*" |
sort | uniq -c | sort -nr > item_count.txt


# calculate co-occurence of items (appearing in the same sparql query)
zcat data/2017-06-12_2017-07-09_uniq_uq.tsv.gz | python3 calculate_cooccurence.py P > cooccurence_P.txt
zcat data/2017-06-12_2017-07-09_uniq_uq.tsv.gz | python3 calculate_cooccurence.py Q > cooccurence_Q.txt